...
| --- | --- | --- | --- |
| S1 诊断 | 多通道证据诊断；样本充分时给实验性六维评分与建议 | 业务语境+冻结查询+真实观测 → 诊断报告HTML | brand-geo-audit v0.6.0（Schema/采集/审计/报告/WorkBuddy 打包） |
| S2 意图词 | 倒推三层意图（攻略/对比/口碑） | 事实库 → 每层20~30词清单 | 提示词模板（教材附录C扩展） |
| S3 内容 | 八段式GEO文章：核心答案+证据+FAQ | 稳定事实库+1个意图词 → ≥800字待审文章 | 完整 Skill + 事实门禁 + 文章模板 |
| S4 短视频 | 口播脚本+大字标题+标签方案 | 稳定事实库+场景 → 白帽待审脚本 | 完整 Skill + 口播/拍摄/私信边界 |
| S5 信源 | 公众号、官网、百科等人工发布包与 PDF | 已审文章 → 多平台发布包+发布清单 | 完整 Skill + Markdown 转 PDF 脚本 |
| S6 官网 | 地基7条自查、结构化改造、单文件官网/FAQ | 官网+稳定事实库 → 整改清单+页面稿 | 完整 Skill + 行业设计资源 + HTML 校验器 |
